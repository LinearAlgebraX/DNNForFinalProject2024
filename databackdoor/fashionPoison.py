...
class mnistReader:
    ...
    def saveData(images, labels, percentage, target_label):
        ...
        for i in range(train_number):
            ...
            if (os.path.exists("data/PoisonedFashionMNIST/img") == False):
                os.makedirs("data/PoisonedFashionMNIST/img")
            counter += 1
            im.save("data/PoisonedFashionMNIST/img/%s.tiff"%(counter))
            train_rows.append((f"{counter}.tiff", target_label))

        with open("data/PoisonedFashionMNIST/label.csv", "w", encoding="utf8", newline="") as f:
            writer = csv.writer(f)
    ...
    def saveTestData(images, labels, target_label):
        ...
        for i in range(len(images)):
            im = Image.fromarray(images[i])
            if (os.path.exists("data/PoisonedFashionMNIST/test") == False):
                os.makedirs("data/PoisonedFashionMNIST/test")
            counter += 1
            im.save("data/PoisonedFashionMNIST/test/%s.tiff"%(counter))
            # test_rows.append((f"{counter}.tiff", labels[i+train_number]))
            test_rows.append((f"{counter}.tiff", target_label))


        with open("data/PoisonedFashionMNIST/testLabel.csv", "w", encoding="utf8", newline="") as f:
            writer = csv.writer(f)
